...
<a href="{{ route('rm_obat.index') }}" type="button" class="btn btn rouned-3">Rekam Medis Obat</a>
...
<div class="form-search" style="float:right">
    <form action="{{ route('rekam_medis.search') }}" method="get" accept-charset="utf-8">
        <div class="form-group" style="display:flex">
            <input type="search" id="nama" name="nama" class="form-control" placeholder="ID Rekam Medis">
            <button type="submit" class="btn btn-secondary">Search</button>
    </form>
</div></div>
<a href="{{ route('rekam_medis.create') }}" type="button" class="btn btn-success rounded-3">Tambah Rekam Medis</a>
...
<table class="table table-hover mt-2">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('rekam_medis.edit', $data->id_rm) }}" type="button"
                    class="btn btn-warning rounded-3">Ubah</a>
                <!-- TAMBAHKAN KODE DELETE DIBAWAH BARIS INI -->
                <a href="{{ route('rekam_medis.hide', $data->id_rm) }}" type="button"
